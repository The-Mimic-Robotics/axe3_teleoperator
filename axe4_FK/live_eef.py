...
#
# At home (motors ≈ 0°) the arm should be in the arch pose:
#   a2 = +90°  (upper arm UP)
...
MOTOR_CFG = [
    # (motor_name,          sign, offset_deg)
    # All signs -1: +motor → -angle.  Offsets keep home pose correct.
...
# ═══════════════════════════════════════════════════════════════════════
# FK MATH — pure trig, no matrices
# ═══════════════════════════════════════════════════════════════════════

def forward_kinematics(q1, q2, q3, q4):
    """
    Planar 3-link chain + pan.  Returns (eef_xyz[3], chain[5×3]).
...
def motor_deg_to_angles(deg_dict):
    """Calibrated motor degrees → (q1, q2, q3, q4) in radians."""
    q = np.zeros(4)
