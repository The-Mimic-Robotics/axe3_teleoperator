"""UDP transport for AXE leader teleop.
...
class UDPTransport(PoseTransport):
	def __init__(self, ip: str = "127.0.0.1", port: int = 5005, pose_only: bool = True, print_packets: bool = False):
	...
	def publish_eef_pose(self, x, y, z, qw, qx, qy, qz):
		self._debug_print("POSE", (x, y, z, qw, qx, qy, qz))
	...
	def publish_buttons(self, sw, sw2, joy_x=0.0, joy_y=0.0, joy_z=0.0):
		if self._pose_only:
			return
		self._debug_print("BUTTONS", (joy_x, joy_y, joy_z, int(sw), int(sw2)))
